...
def calculate_mifflin_st_jeor(weight_kg: float, height_cm: float, age: int, gender: str, activity_level: str, goal: str):
    """
...
def update_my_profile(
    ...
    for field in ["name", "gender", "age", "height_cm", "weight_kg", "activity_level", "goal"]:
    ...
    if payload.auto_calculate:
        ...
        required_fields = [profile.weight_kg, profile.height_cm, profile.age, profile.gender, profile.activity_level, profile.goal]
        ...
        cals, p, c, f = calculate_mifflin_st_jeor(
            profile.weight_kg, profile.height_cm, profile.age, profile.gender, profile.activity_level, profile.goal
        )
        ...
        if payload.target_calories is not None: profile.target_calories = payload.target_calories
        if payload.target_protein_g is not None: profile.target_protein_g = payload.target_protein_g
        if payload.target_carbs_g is not None: profile.target_carbs_g = payload.target_carbs_g
        if payload.target_fats_g is not None: profile.target_fats_g = payload.target_fats_g
