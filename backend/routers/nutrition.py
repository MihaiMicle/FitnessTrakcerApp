...
def get_log_by_date(
    ...
    t_sat_fats = (
        ...
        if profile and profile.target_saturated_fats_g
        else 20
    )
    ...
    t_potassium = (
        profile.target_potassium_mg if profile and profile.target_potassium_mg else 4000
    )
    t_sodium = (
        profile.target_sodium_mg if profile and profile.target_sodium_mg else 2300
    )
    t_iron = profile.target_iron_mg if profile and profile.target_iron_mg else 8.0
    t_vitamin_d = (
        profile.target_vitamin_d_mcg
        if profile and profile.target_vitamin_d_mcg
        else 25.0
    )
    t_zinc = profile.target_zinc_mg if profile and profile.target_zinc_mg else 11.0
    t_magnesium = (
        profile.target_magnesium_mg
        if profile and profile.target_magnesium_mg
        else 400.0
    )
    t_calcium = (
        profile.target_calcium_mg if profile and profile.target_calcium_mg else 1200.0
    )
    t_cholesterol = (
        profile.target_cholesterol_mg
        if profile and profile.target_cholesterol_mg
        else 300.0
    )
...
def log_daily_nutrition(
    ...
    if existing_log:
        ...
        if payload.total_water_ml is not None:
            existing_log.total_water_ml = payload.total_water_ml
...
def delete_meal(
    ...
    if log:
        ...
        log.total_saturated_fats_g = max(
            0.0, log.total_saturated_fats_g - meal.saturated_fats_g
        )
        ...
        log.total_cholesterol_mg = max(
            0.0, log.total_cholesterol_mg - meal.cholesterol_mg
        )
